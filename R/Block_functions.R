...
#'
#' `CreateBlock` creates the complete HDF5 on-disk data structure
#'
...
#' specifying the chromosomes present in this particular Block file.
#'        \item other metadata tables.
#'    }
#'}
#' @return This function will generate the target Block file. Upon completion,
#' the function will provide the path to the created/tracked HDF file.
#'
#' @examples
#' Bintable.path <- system.file("extdata",
#' "Bintable_40kb.txt", package = "HiCBlocks")
#' Chromosomes <- "chr19"
#' Path_to_cached_file <- CreateBlock(ChromNames = Chromosomes,
...
#' }
#'
CreateBlock <- function(ChromNames=NULL, BinTable=NULL, bin.delim="\t",
    ...
    Tracked_name <- ._Get_Block_hashname(Block = Output.Filename)
    ...
    if(Dir.path == "."){
        ...
        if(Block_is_tracked(Block = Output.Filename)){
            Working.File <- Block_path_to_file(Output.Filename)
            CreateNewCacheObject <- TRUE
        }else{
            Working.File <- ._Create_new_cached_file(Cache.dir = Cache.dir,
                Block.path = Output.Filename)
            CreateNewCacheObject <- FALSE
        }
    }else{
    ...
    if(file.exists(HDF.File)){
        ...
        Block_untrack_block(Block = Output.Filename)
    ...
    ._Block_WriteDataFrame_(Block = HDF.File, 
        Path = c(Root.folders['metadata']), 
        name = Reference.object$metadata.chrom.dataset, 
        object = Chrom.info.df)
    ._Block_Add_Ranges_(Group.path = Create_Path(c(Root.folders['ranges'],
        Reference.object$hdf.bintable.ranges.group)), Block = HDF.File,
    ...
    if(CreateCacheObject){
        Block_track_blocks(Block = HDF.File)
    }
...
#'
#' `CreateBlock_from_mcool` is a wrapper on CreateBlock which creates the 
#' Block data structure from an mcool file.
#'
...
#' If provided will only create a Block for these
#' chromosomes (both cis & trans).
#'
#' @param binsize \strong{Optional}.
#' The binsize to select from an mcool file.
#'
#' @inheritParams Block_load_data_from_mcool
#'
#' @inheritParams CreateBlock
#'
#' @return This function will generate the target Block file. Upon completion,
#' the function will provide the path to the created/tracked HDF file.
#'
...
#'
#' Output.block <- paste("./H1-hESC-HiC-4DNFI7JNCNFB-10000",
#' "ICE-normalised-chr1.block",sep = "-")
#' mcool <- "./H1-hESC-HiC-4DNFI7JNCNFB.mcool"
#'
#' CreateBlock_from_mcool(Block = Output.block,
#' mcool = mcool,
#' binsize = 10000,
#' chrs = "chr1")
#'
#' }
#'
#' @seealso \code{\link{Block_load_data_from_mcool}} to load data from
#' the mcool to a Block store.
#'
CreateBlock_from_mcool <- function(Block = NULL, mcool = NULL, binsize = NULL,
    ...
    resolutions <- Block_list_mcool_resolutions(mcool = mcool)
    ...
    if(!is.null(resolutions)){
        ...
        if(!(as.character(binsize) %in% resolutions)){
            stop("all binsizes were not found in this mcool file. See all",
                " resolutions available with Block_list_mcool_resolutions\n")
        }
    }
    ...
    RetVar <- CreateBlock(ChromNames=ChromNames,
        BinTable=mcool_bintable_ranges, Output.Filename=Block,
...
#'
#' `Block_list_mcool_resolutions` lists all available resolutions in the mcool
#' file.
#'
...
#'
Block_list_mcool_resolutions <- function(mcool = NULL){
    return(mcool_list_resolutions(mcool = mcool))
}

#' Get all available normalisations in an mcool file.
#'
#' `Block_list_mcool_normalisations` lists the names available for
...
#'
#' @examples
#' Block_list_mcool_normalisations()
#'
Block_list_mcool_normalisations <- function(names.only = FALSE){
    Reference.object <- GenomicMatrix$new()
    if(names.only){
...
#'
#' `Block_mcool_normalisation_exists` checks if a particular normalisation
...
#'
#' mcool <- "./H1-hESC-HiC-4DNFI7JNCNFB.mcool"
#' Block_mcool_normalisation_exists(mcool = mcool,
#' norm.factor = "Iterative-Correction",
#' binsize = 10000)
#'
#' }
#'
Block_mcool_normalisation_exists <- function(mcool = NULL, norm.factor = NULL,
    binsize = NULL){
    Reference.object <- GenomicMatrix$new()
    Norm.factors <- Block_list_mcool_normalisations()
    ...
    if(!is.null(resolutions) & !(binsize %in% resolutions)){
        stop("binsize not found in mcool file.",
            ...
            " with Block_list_mcool_resolutions.\n")
    }
    if(!is.null(resolutions)){
    ...
    Handler <- ._Block_Get_Something_(Group.path = Bintable.group.path,
...
#'
#' `Block_get_chrominfo` fetches the associated chrominfo table for the
#' Block it is associated to.
#'
#' @param Block \strong{Required}.
#' A string specifying the path to the Block store created with CreateBlock.
#'
...
#' Block_get_chrominfo(Block = Block.file)
Block_get_chrominfo <- function(Block = NULL){
    Reference.object <- GenomicMatrix$new()
    Dataset <- ._Block_Get_Something_(
        Group.path = Reference.object$hdf.metadata.root,
        Block = Block, Name = Reference.object$metadata.chrom.dataset,
        return.what = "data")
    return(Dataset)
}
...
#'
#' `Block_make_ranges` creates a GRanges object from the provided arguments
#' 
...
#' Test_ranges <- Block_make_ranges(Chrom = Chrom, Start = Start, End = End)
#'
Block_make_ranges = function(Chrom=NULL, Start=NULL, End=NULL, Strand=NULL,
    Names=NULL){
    Reference.object <- GenomicMatrix$new()
...
#' Store a ranges object in the Block store.
#'
#' `Block_add_ranges` loads a GRanges object into the Block store.
#'
#' With this function it is possible to associate other ranges objects with the
#' Block store. If metadata columns are present, the are also loaded into the
#' Block store. Although not explicitly asked for, the metadata columns should
...
#' An object of class ranges specifying the ranges to store in the Block.
#'
#' @param rangekey \strong{Required}.
#' The name to use for the ranges within the Block store.
#'
...
#' remove.existing = TRUE)
#'
#' Block.file <- Block_path_to_file("test.hdf")
#' Chrom <- c("chrS","chrS","chrS","chrS","chrS")
#' Start <- c(10000,20000,40000,50000,60000)
#' End <- c(10001,20001,40001,50001,60001)
#' Test_ranges <- Block_make_ranges(Chrom = Chrom, Start = Start, End = End)
#' Block_add_ranges(Block = Block.file, ranges = Test_ranges,
...
#' }
#'
Block_add_ranges = function(Block = NULL, ranges = NULL, rangekey = NULL,
    remove.existing = TRUE){
    Reference.object <- GenomicMatrix$new()
    ...
    if(Block_rangekey_exists(Block = Block, rangekey = rangekey)){
        # if(!remove.existing){
    ...
    ._Block_Add_Ranges_(
        ...
        Block = Block, ranges.df = Ranges.df.coords, name = rangekey,
        mcol.list = Metadata.list)
    return(TRUE)
}

#' List the ranges tables stored within the Block.
#'
#' `Block_list_rangekeys` lists the names of all ranges associated to a Block.
#' 
...
#'
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' Block_list_rangekeys(Block = Block.file)
#'
Block_list_rangekeys = function(Block = NULL){
    Reference.object <- GenomicMatrix$new()
    Handler <- ._Block_Get_Something_(
        Group.path = Create_Path(Reference.object$hdf.ranges.root),
        Block = Block, return.what = "group_handle")
    GroupList <- h5ls(Handler, datasetinfo = FALSE, recursive = FALSE)[,"name"]
    return(GroupList)
}

#' Check to see if the Block contains a ranges with a certain name.
#'
#' `Block_rangekey_exists` checks for the presence of a particular ranges with
#' a certain name.
#'
#' @param rangekey \strong{Required}.
...
#'
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' Block_rangekey_exists(Block = Block.file, rangekey = "Bintable")
#'
Block_rangekey_exists = function(Block = NULL, rangekey = NULL){
    Keys <- Block_list_rangekeys(Block = Block)
...
#' name
#'
#' `Block_list_ranges_mcols` will list the metadata columns of the specified
#' ranges if it is present in the Block store.
#'
...
#' @examples
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' Block_list_ranges_mcols(Block = Block.file, rangekey = "test_ranges")
#'
Block_list_ranges_mcols = function(Block = NULL, rangekey = NULL){
    Reference.object <- GenomicMatrix$new()
    RangeKeys <- Block_list_rangekeys(Block = Block)
    if(!is.null(rangekey)){
        if(!Block_rangekey_exists(Block = Block, rangekey = rangekey)){
...
#' List the matrix pairs present in the Block store.
#'
#' `Block_list_matrices` will list all chromosomal pair matrices from the 
#' Block store, with their associated filename, value range, done status 
#' and sparse
#'
#' @inheritParams Block_get_chrominfo
#'
...
#' @examples
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' Block_list_matrices(Block = Block.file)
#'
Block_list_matrices = function(Block = NULL){
    Reference.object <- GenomicMatrix$new()
...
#'
#' `Block_get_ranges` will get a ranges object if present in the Block store
...
#' other than "Bintable", and had been added using Block_add_ranges the width
...
#' @examples
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' Block_get_ranges(Block = Block.file, chr = "chr19", rangekey = "Bintable")
#'
Block_get_ranges = function(Block = NULL, chr = NULL, rangekey = NULL){
    Reference.object <- GenomicMatrix$new()
    transformlist <- list("strand" = strand, "seqlevels" = seqlevels,
        "seqlengths" = seqlengths, "width" = width)
    if(is.null(rangekey) | is.null(Block)){
        stop("rangekey and Block cannot remain empty!\n")
    }
    if(!Block_rangekey_exists(Block = Block, rangekey = rangekey)){
        stop("rangekey not found!")
    }
    ...
    Index <- NULL
    if(!is.null(chr)){
        chromosomes <- ._Block_Get_Something_(Group.path = Create_Path(
        ...
        return.what = "data")
        Lengths <- ._Block_Get_Something_(Group.path = Create_Path(
            c(Reference.object$hdf.ranges.root,
                rangekey)
            ),
        Block = Block,
    ...
    Dataset <- ._Block_Get_Something_(Group.path = Create_Path(
        c(Reference.object$hdf.ranges.root, rangekey)),
        Block = Block, Name = Reference.object$hdf.ranges.dataset.name,
        Start = Start, Stride = Stride,
        Count = Count, return.what = "data")
    Dataset <- Block_make_ranges(Chrom = Dataset[,'chr'],
        Start = Dataset[,'start'], End = Dataset[,'end'])
    MCols <- Block_list_ranges_mcols(Block = Block, rangekey = rangekey)
    if(is.data.frame(MCols)){
        ...
        if(length(GRangesCols) > 0){
            ...
            for(x in FUN.names){
                FUN <- transformlist[[x]]
                FUN(Dataset) <- ._Block_Get_Something_(Group.path = Create_Path(
                    c(Reference.object$hdf.ranges.root, rangekey)),
                Block = Block, Name = x, Start = m.start, Stride = m.stride,
...
#'
#' `Block_get_bintable` makes a call to \code{\link{Block_get_ranges}} to
#' retrieve the binning table of the associated Block store. This is equivalent
#' to passing the argument rangekey = "bintable" in
#' \code{\link{Block_get_ranges}}
#'
#' @inheritParams Block_get_chrominfo
#'
#' @param chr \strong{Optional}.
...
#' table associated to the Block store.
#'
#' @examples
#'
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' Block_get_bintable(Block = Block.file)
#'
#' @seealso Block_get_ranges
Block_get_bintable = function(Block = NULL, chr = NULL){
    Reference.object <- GenomicMatrix$new()
    Table <- Block_get_ranges(Block = Block, chr = chr,
...
#'
#' `Block_fetch_range_index` constructs a ranges object using
#' \code{\link{Block_make_ranges}}, creates an overlap operation using
...
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' Test_Run <- Block_fetch_range_index(Block = Block.file, chr = Chrom,
#' start = Start, end = End)
#' Test_Run$Indexes[[1]]
#'
Block_fetch_range_index = function(Block = NULL, chr = NULL, start = NULL,
    ...
    if(is.null(chr) | is.null(start) | is.null(end) | is.null(Block)){
        stop("Chrom, start, end and Block cannot be empty\n")
    }
    ChromInfo <- Block_get_chrominfo(Block = Block)
    ...
    OverlapByChromosome.list <- lapply(Unique.chromosomes,function(cur.chr){
        ...
        SubjectRanges <- Block_get_bintable(Block = Block, chr = cur.chr)
        if( any(!(Cur.end <= max(end(SubjectRanges)) &
            Cur.Start >= min(start(SubjectRanges)))) ){
            stop("Start or end is out of ranges for Bintable\n")
        }
        QueryRanges <- Block_make_ranges(Chrom=Cur.Chrom, Start=Cur.Start,
...
#'
#' `Block_return_region_position` takes as input a human-readable coordinate
...
#' overlapping regions in the bintable associated the Block store.
#'
#' @examples
#'
#' Coordinate <- "chr19:1:1000000"
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' Test_Run <- Block_return_region_position(Block = Block.file,
#' region = Coordinate)
#'
#' \dontrun{
#'
...
#'
#' }
Block_return_region_position = function(Block = NULL, region=NULL){
    ...
    region.ranges<-Block_fetch_range_index(Block = Block, chr=region.chr,
        start=region.start, end=region.stop, type="within")
    Vector.coordinates <- region.ranges$Indexes[[1]]
    return(Vector.coordinates)
}

#' Load a NxM dimensional matrix into the Block store.
#'
#' @inheritParams Block_get_chrominfo
#'
...
#' Block.file <- Block_path_to_file(Block = "test.hdf")
#' Block_load_matrix(Block = Block.file, chr1 = "chr19", chr2 = "chr19",
#' matrix.file = Matrix.file, delim = " ", exec = "cat",
#' remove.prior = TRUE)
#'
Block_load_matrix = function(Block = NULL, chr1 = NULL, chr2 = NULL,
    matrix.file = NULL, delim = " ", exec = NULL, remove.prior = FALSE,
    num.rows = 2000, is.sparse = FALSE, sparsity.bins = 100){

    Reference.object <- GenomicMatrix$new()
    ListVars <- list(Block = Block, chr1 = chr1, chr2 = chr2,
    ...
    RetVar <- ._ProcessMatrix_(Block = Block, Matrix.file = matrix.file,
...
#' @inheritParams Block_get_chrominfo
#'
#' @inheritParams Block_load_matrix
#'
#' @param chr \strong{Required}.
#' A character vector of length 1 specifying the chromosome corresponding to
...
#' row.names = FALSE, col.names = FALSE)
#' Block.file <- Block_path_to_file("test.hdf")
#' Block_load_cis_matrix_till_distance(Block = Block.file, chr = "chr19",
#' matrix.file = Matrix.file, delim = " ", distance = 200, remove.prior = TRUE)
#'
Block_load_cis_matrix_till_distance = function(Block = NULL, chr = NULL,
    matrix.file = NULL, delim = " ", distance = NULL, remove.prior = FALSE,
    num.rows = 2000, is.sparse = FALSE, sparsity.bins = 100){

    Reference.object <- GenomicMatrix$new()
    ListVars <- list(Block = Block, chr = chr, matrix.file = matrix.file,
    ...
    lapply(seq_along(ListVars[c("Block","chr","matrix.file","distance")]),
        function(x){
        if(is.null(ListVars[[x]])){
            stop(names(ListVars[x]),"has no value.\n")
        }
    })
    if(!Block_matrix_exists(Block = Block, chr1 = chr,
        chr2 = chr)){
        stop("Provided chromosomes do not exist in the chrom table\n")
    }
    if(Block_matrix_isdone(Block = Block, chr1 = chr,
    ...
    RetVar <- ._Process_matrix_by_distance(Block = Block,
...
#' Load a NxN dimensional matrix into the Block store from an mcool file.
#'
...
#' @inheritParams Block_load_matrix
#'
#' @inheritParams CreateBlock_from_mcool
#' @param mcool \strong{Required}.
...
#'
#' Output.block <- paste("./H1-hESC-HiC-4DNFI7JNCNFB-10000",
#' "ICE-normalised-chr1.hicblock",sep = "-")
#' mcool <- "./H1-hESC-HiC-4DNFI7JNCNFB.mcool"
#'
#' CreateBlock_from_mcool(Block = Output.block,
#' mcool = mcool,
...
#' chrs = "chr1")
#'
#' Block_load_data_from_mcool(Block = Output.block, mcool = mcool,
...
#'
#'
#' @seealso \code{\link{CreateBlock_from_mcool}} to create matrix from an mcool
#' file, \code{\link{Block_list_mcool_resolutions}} to list available
#' resolutions in an mcool file, \code{\link{Block_list_mcool_normalisations}}
#' to list available normalisation factors in the mcool file.
#'
Block_load_data_from_mcool <- function(Block = NULL, mcool = NULL, chr1 = NULL,
    ...
    if(!Block_matrix_exists(Block = Block, chr1 = chr1,
        chr2 = chr2)){
    ...
    if(!is.null(Norm.factor)){
        if(!Block_mcool_normalisation_exists(mcool = mcool,
    ...
    RetVar <- ._Process_mcool(Block = Block, File = mcool,
...
#' @examples
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' Block_matrix_isdone(Block = Block.file, chr1 = "chr19", chr2 = "chr19")
#'
Block_matrix_isdone = function(Block = NULL, chr1 = NULL, chr2 = NULL){
    Reference.object <- GenomicMatrix$new()
    Matrix.list <- Block_list_matrices(Block = Block)
    if(!Block_matrix_exists(Block = Block, chr1 = chr1, chr2 = chr2)){
...
#' @examples
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' Block_matrix_issparse(Block = Block.file, chr1 = "chr19", chr2 = "chr19")
#'
Block_matrix_issparse = function(Block = NULL, chr1 = NULL, chr2 = NULL){
    Reference.object <- GenomicMatrix$new()
    Matrix.list <- Block_list_matrices(Block = Block)
    if(!Block_matrix_exists(Block = Block, chr1 = chr1, chr2 = chr2)){
        stop("chr1 chr2 pairs were not found\n")
...
#' 
#' `Block_matrix_maxdist` will return this parameter.
#'
#' @inheritParams Block_get_chrominfo
#'
#' @inheritParams Block_load_matrix
...
#' @examples
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' Block_matrix_maxdist(Block = Block.file, chr1 = "chr19", chr2 = "chr19")
#'
Block_matrix_maxdist = function(Block = NULL, chr1 = NULL, chr2 = NULL){
    Reference.object <- GenomicMatrix$new()
    Matrix.list <- Block_list_matrices(Block = Block)
    if(!Block_matrix_exists(Block = Block, chr1 = chr1, chr2 = chr2)){
        stop("chr1 chr2 pairs were not found\n")
    }
    if(!Block_matrix_isdone(Block = Block, chr1 = chr1, chr2 = chr2)){
...
#' @examples
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' Block_matrix_exists(Block = Block.file, chr1 = "chr19", chr2 = "chr19")
#'
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' Block_matrix_exists(Block = Block.file, chr1 = "chr19", chr2 = "chr20")
#'
Block_matrix_exists = function(Block = NULL, chr1 = NULL, chr2 = NULL){
    ChromInfo.df <- Block_get_chrominfo(Block = Block)
...
#' @examples
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' Block_matrix_minmax(Block = Block.file, chr1 = "chr19", chr2 = "chr19")
#'
Block_matrix_minmax = function(Block = NULL, chr1 = NULL, chr2 = NULL){
    Reference.object <- GenomicMatrix$new()
    Matrix.list <- Block_list_matrices(Block = Block)
    if(!Block_matrix_exists(Block = Block, chr1 = chr1, chr2 = chr2)){
        stop("chr1 chr2 pairs were not found\n")
...
#' @examples
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' Block_matrix_dimensions(Block = Block.file, chr1 = "chr19", chr2 = "chr19")
#'
Block_matrix_dimensions = function(Block=NULL, chr1=NULL, chr2=NULL){
    if(!Block_matrix_exists(Block = Block, chr1 = chr1, chr2 = chr2)){
        stop("chr1 chr2 pairs were not found\n")
    }
    Reference.object <- GenomicMatrix$new()
...
#' @examples
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' Block_matrix_filename(Block = Block.file, chr1 = "chr19", chr2 = "chr19")
#'
Block_matrix_filename = function(Block = NULL, chr1 = NULL, chr2 = NULL){
    Reference.object <- GenomicMatrix$new()
    Matrix.list <- Block_list_matrices(Block = Block)
    if(!Block_matrix_exists(Block = Block, chr1 = chr1, chr2 = chr2)){
        stop("chr1 chr2 pairs were not found\n")
...
#'
#' `Block_get_values_by_distance` can fetch values with or without
...
#' @examples
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' Block_get_values_by_distance(Block = Block.file, chr = "chr19",
#' distance = 0)
#'
...
#'
#' @seealso \code{\link{Block_get_matrix_within_coords}} to get matrix by
#' using matrix coordinates, \code{\link{Block_fetch_row_vector}} to get values
#' in a certain row/col and subset them, \code{\link{Block_get_vector_values}}
#' to get values using matrix coordinates, \code{\link{Block_get_matrix}} to
#' get matrix by using matrix coordinates.
#'
Block_get_values_by_distance = function(Block = NULL, chr = NULL,
    distance  = NULL, constrain.region=NULL,batch.size=500,FUN=NULL){
    if(any(vapply(list(Block,chr,distance),is.null,TRUE))) {
        stop("Block, chr, distance cannot be NULL.\n")
    }
    Reference.object <- GenomicMatrix$new()
    ChromInfo <- Block_get_chrominfo(Block = Block)
    if(!Block_matrix_exists(Block = Block, chr1 = chr, chr2 = chr) &
        !Block_matrix_isdone(Block = Block, chr1 = chr, chr2 = chr)){
        stop("Chromosome is not listed or has not been ",
            "loaded in this HDF file.\n")
    }
    if(any(vapply(list(Block,chr,distance),length,1) > 1)){
        stop("Block, chr and distance can only have values of length 1.\n")
    }
    Nrow <- (ChromInfo[ChromInfo$chr == chr,"nrow"]-1)
    if(any(distance > Nrow, distance < 0)){
        stop("distance must range between 0 and",Nrow,"\n")
    }
    Max.dist <- Block_matrix_maxdist(Block = Block, chr1 = chr, chr2 = chr)
    ...
    if(!is.null(constrain.region)){
        Vector.coordinates <- Block_return_region_position(Block = Block,
    ...
    DistancesVector.list <- lapply(seq_len(length(Counts)),function(x){
        ...
        diag(._Block_Get_Something_(Group.path = Path, Block = Block,
...
#'
#' `Block_get_matrix_within_coords` will fetch a matrix subset after
#' creating an overlap operation between both regions and the bintable
#' associated to the Block store.
#' This function calls \code{\link{Block_get_matrix}}.
#'
#' @inheritParams Block_get_chrominfo
#'
...
#' @examples
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' Block_get_matrix_within_coords(Block = Block.file,
...
#' FUN = median)
#' 
#' @seealso \code{\link{Block_get_matrix}} to get matrix by using matrix
#' coordinates, \code{\link{Block_get_values_by_distance}} to get values
#' separated at a certain distance, \code{\link{Block_fetch_row_vector}}
#' to get values in a certain row/col and subset them,
#' \code{\link{Block_get_vector_values}} to get values using matrix
#' coordinates.
Block_get_matrix_within_coords = function(Block = NULL, x.coords=NULL,
    y.coords=NULL, force = FALSE, FUN=NULL){
    type <- "within"
    if( (is.null(x.coords)) | (is.null(y.coords)) ){
        stop("x.coords, y.coords and Block cannot be NULL")
    }
    ...
    chr1.ranges <- Block_fetch_range_index(Block = Block, chr = chr1,
    start = chr1.start, end = chr1.stop, names=NULL, type=type)
    chr2.ranges <- Block_fetch_range_index(Block = Block, chr = chr2,
    ...
    Matrix <- Block_get_matrix(Block = Block, chr1=chr1, chr2=chr2,
...
#'
#' `Block_get_matrix` will fetch a matrix subset between row values
...
#' is returned.
#'
#' @inheritParams Block_get_matrix_within_coords
#'
...
#' @examples
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' Block_get_matrix(Block = Block.file, chr1 = "chr19", chr2 = "chr19",
#' x.vector = c(1:10), y.vector = c(1:10))
#'
#' @seealso \code{\link{Block_get_matrix_within_coords}} to get matrix by using
#' matrix genomic coordinates, \code{\link{Block_get_values_by_distance}} to
#' get values separated at a certain distance,
#' \code{\link{Block_fetch_row_vector}} to getvalues in a certain row/col and
#' subset them, \code{\link{Block_get_vector_values}} to get values using
#' matrix coordinates.
Block_get_matrix = function(Block = NULL, chr1=NULL, chr2=NULL, x.vector=NULL,
    ...
    Matrix <- Block_get_vector_values(Block = Block, chr1=chr1, chr2=chr2,
...
#'
#' `Block_fetch_row_vector` will fetch any given rows from a matrix. If
...
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' Test_Run <- Block_fetch_row_vector(Block = Block.file,
#' chr1 = "chr19", chr2 = "chr19", by = "ranges", vector = Coordinate,
#' regions = c("chr19:1:1000000", "chr19:40001:2000000"))
#'
#' @seealso \code{\link{Block_get_matrix_within_coords}} to get matrix by
#' using matrix genomic coordinates, \code{\link{Block_get_values_by_distance}}
#' to get values separated at a certain distance,
#' \code{\link{Block_fetch_row_vector}} to get values in a certain row/col and
#' subset them, \code{\link{Block_get_matrix}} to get matrix by using
#' matrix coordinates.
#'
Block_fetch_row_vector = function(Block = NULL, chr1=NULL, chr2=NULL, by=NULL,
    ...
    ChromInfo <- Block_get_chrominfo(Block = Block)
    ChromosomeList <- ChromInfo[,"chr"]
    max.dist <- Block_matrix_maxdist(Block = Block, chr1 = chr1, chr2 = chr2)
    ...
    chr1.ranges <- Block_get_bintable(Block = Block, chr=chr1)
    chr2.ranges <- Block_get_bintable(Block = Block, chr=chr2)
    if(flip){
        chr1.ranges <- Block_get_bintable(Block = Block, chr=chr2)
        chr2.ranges <- Block_get_bintable(Block = Block, chr=chr1)
    }
    if(by=="ranges"){
    ...
    Vector.values<-lapply(Seq.indices,function(ind){
        ...
        if(PikaPika){
            region <- regions[ind]
            y <- Block_return_region_position(Block = Block, region = region)
        }else{
        ...
        Values <- Block_get_vector_values(Block = Block, chr1=chr1, chr2=chr2,
...
#'
#' `Block_get_vector_values` is the base function being used by all
...
#' @examples
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' Block_get_vector_values(Block = Block.file, chr1 = "chr19",
...
#'
Block_get_vector_values = function(Block = NULL, chr1=NULL, chr2=NULL,
    ...
    Group.path <- Create_Path(c(Reference.object$hdf.matrices.root,
        chr1, chr2))
    Vector <- ._Block_Get_Something_(Group.path = Group.path, Block = Block,
        Name = Reference.object$hdf.matrix.name, Start = Start,
        Stride = Stride, Count = Count, return.what = "data")
    if(is.null(FUN)){
        return(Vector)
    }else{
        return(FUN(Vector))
...
#' Get the matrix metadata columns in the Block store.
#'
#' `Block_get_matrix_mcols` will get the specified matrix metadata column.
#'
#' @inheritParams Block_get_chrominfo
#'
#' @inheritParams Block_load_matrix
...
#' @examples
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' Block_get_matrix_mcols(Block = Block.file, chr1 = "chr19", chr2 = "chr19",
#' what = "bin.coverage")
Block_get_matrix_mcols = function(Block = NULL, chr1 = NULL, chr2 = NULL,
    what = NULL){
    Reference.object <- GenomicMatrix$new()
    Meta.cols <- Reference.object$hdf.matrix.meta.cols()
    if(any(is.null(c(Block,chr1,chr2,what)))){
        stop("Block, chr1, chr2, what cannot be NULL.\n")
    }
    if(!Block_matrix_exists(Block = Block, chr1 = chr1, chr2 = chr2)){
    ...
    if(!Block_matrix_issparse(Block = Block, chr1 = chr1, chr2 = chr2) & what
        == Meta.cols["sparse"]){
...
#' List the matrix metadata columns in the Block store.
#'
#' `Block_get_matrix_mcols` will list the names of all matrix metadata 
#' columns.
#'
#' @inheritParams Block_get_chrominfo
#'
...
#' @examples
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' Block_list_matrix_mcols(Block = Block.file, chr1 = "chr19", chr2 = "chr19")
Block_list_matrix_mcols = function(Block = NULL, chr1 = NULL, chr2 = NULL){
    Reference.object <- GenomicMatrix$new()
    Meta.cols <- Reference.object$hdf.matrix.meta.cols()
    return(Meta.cols)
}
...
#' List all Blocks that have been created with .
#'
#' `Block_list_tracked_blocks` will list all Block objects that are being
#' tracked.
#'
#' @param detailed \strong{Optional}
#' If FALSE, produces a data.frame of file paths pointing to the
#' Block objects. If TRUE, produces a tibble containing additional information
...
#' Block objects. If tibble will contain additional information related to
#' the tracking. For details see parameter detailed.
#'
#' @examples
#' Block_list_tracked_blocks()
Block_list_tracked_blocks = function(detailed = FALSE,
    preserve.col.names = FALSE){
    Reference.object <- GenomicMatrix$new()
    Cache.dir <- ._Get_cachedir()
    Info.tib <- bfcquery(x = Cache.dir,
        query = paste(".",Reference.object$block.extension,sep = ""),
        field = "rpath")
    if(nrow(Info.tib) == 0){
        return(NULL)
    }
...
#'
#' `Block_is_tracked` checks if the provided Block file is being
#' tracked or not.
#'
#' @param Block \strong{required}
#' Path to the Block file to check.
#' 
...
#' @examples
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' Block_is_tracked(Block = Block.file)
Block_is_tracked = function(Block = NULL){
    Reference.object <- GenomicMatrix$new()
    Cache.dir <- ._Get_cachedir()
    Tracked_name <- ._Get_Block_hashname(Block = Block)
...
#' Check if a Block is being tracked.
#'
#' `Block_track_blocks` will start tracking the provided Block file.
#'
#' @param Block \strong{required}
#' Path to the Block file to track.
#' 
...
#' @examples
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' if(Block_is_tracked(Block.file)) {
#'     Block_untrack_block(Block = Block.file)
#'     Block_track_blocks(Block = Block.file)
#' }else{
#'     Block_track_blocks(Block = Block.file)
#' }
Block_track_blocks = function(Block = NULL){
    Reference.object <- GenomicMatrix$new()
    Cache.dir <- ._Get_cachedir()
    Dir.path <- normalizePath(dirname(Block))
    Filename <- basename(Block)
    Tracked_name <- ._Get_Block_hashname(Block)
    if(Block_is_tracked(Block = Block)){
        stop("This Block object is already being tracked!")
    }
...
#' Check if a Block is being tracked.
#'
#' `Block_untrack_block` will stop tracking the provided Block file,
#' if it exists.
#'
#' @param Block \strong{required}
#' Path to the Block file to untrack.
#'
...
#'     Block_untrack_block(Block = Block.file)
#' }
Block_untrack_block = function(Block = NULL){
    Reference.object <- GenomicMatrix$new()
    Cache.dir <- ._Get_cachedir()
    if(!(Reference.object$cache.metacol.hashname %in%
            bfcquerycols(Cache.dir))){
        stop("Tracking of Block objects is yet to be initialized!")
    }
    Tracked_name <- ._Get_Block_hashname(Block)
    ...
    if(Block_is_tracked(Block = Block)){
        ...
        stop("Block is not being tracked!")
    }
}
...
#' Check if a Block is being tracked.
#'
#' `Block_path_to_file` will query the tracking cache and return the complete
#' path to the file that is being tracked.
#'
#' @param Block \strong{required}
#' Path to the Block file to locate.
#'
...
#'
#' Block.file <- "test.hdf"
#' Block_path_to_file(Block = Block.file)
Block_path_to_file = function(Block = NULL){
    if(!(Block_is_tracked(Block))){
        stop("The provided Block is not being tracked")
    }
    Tracked_name <- ._Get_Block_hashname(Block)
    Working.File.df <- Block_list_tracked_blocks()
