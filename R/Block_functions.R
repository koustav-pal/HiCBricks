...
CreateBlock <- function(ChromNames=NULL, BinTable=NULL, bin.delim="\t",
    ...
    ._Block_WriteDataFrame_(Block = HDF.File, Path = c(Root.folders['metadata']), 
...
#' 
#' `CreateBlock_from_mcool` is a wrapper on CreateBlock which creates the Block 
...
CreateBlock_from_mcool <- function(Block = NULL, mcool = NULL, binsize = NULL, 
    ...
    RetVar <- CreateBlock(ChromNames=ChromNames, BinTable=mcool_bintable_ranges, 
        Output.Filename=Block, remove.existing = remove.existing)
    return(RetVar)
}
...
#' 
#' `Block_list_matrices` will list all chromosomal pair matrices from the Block
...
#' 
#' `Block_get_ranges` will get a ranges object if present in the Block store and
...
#' @examples
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' Block_get_values_by_distance(Block = Block.file, chr = "chr19", distance = 0)
#' 
...
#' 
#' Block.file <- system.file("extdata", "test.hdf", package = "HiCBlocks")
#' Block_get_values_by_distance(Block = Block.file, chr = "chr19", distance = 4,
#' FUN = Failsafe_median)
#' 
#' @seealso \code{\link{Block_get_matrix_within_coords}} to get matrix by using 
#' matrix coordinates, \code{\link{Block_fetch_row_vector}} to get values in a 
#' certain row/col and subset them, \code{\link{Block_get_vector_values}} to get 
#' values using matrix coordinates, \code{\link{Block_get_matrix}} to get matrix 
...
#' separated at a certain distance, \code{\link{Block_fetch_row_vector}} to get
#' values in a certain row/col and subset them, 
#' \code{\link{Block_get_vector_values}} to get values using matrix coordinates.
...
#' 
#' @seealso \code{\link{Block_get_matrix_within_coords}} to get matrix by using 
#' matrix genomic coordinates, \code{\link{Block_get_values_by_distance}} to get 
#' values separated at a certain distance, \code{\link{Block_fetch_row_vector}} 
...
#' \code{\link{Block_get_matrix}} to get matrix by using matrix coordinates
#' 
...
#' 
Block_get_vector_values = function(Block = NULL, chr1=NULL, chr2=NULL, xaxis=NULL,
...
#' 
#' `Block_get_matrix_mcols` will list the names of all matrix metadata columns. 
#' 
#' @inheritParams Block_get_chrominfo
#' 
#' @inheritParams Block_load_matrix
...
#' 
#' `Block_list_tracked_blocks` will list all Block objects that are being tracked. 
#' 
...
#' 
#' `Block_is_tracked` checks if the provided Block file is being tracked or not. 
#' 
#' @param Block \strong{required}
...
#' Check if a Block is being tracked.
#' 
#' `Block_untrack_block` will stop tracking the provided Block file, if it exists. 
#' 
#' @param Block \strong{required}
