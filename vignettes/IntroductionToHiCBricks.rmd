...
> **Technical note**
When a _Brick object_ is created, the user is required to provide an argument specifying the path to the HDF that will be created. As per Bioconductor core-team recommendations, the users should always provide an explicit full path instead than just a filename assuming it will be written in the working directory.
To enforce compliance with this Bioconductor recommendation, if the brick object is created (with `CreateBrick` function) without a full path in the filename (`Output.Filename` parameter), then the HDF file is saved into the Cache directory rather than in the current working directory. This will typically be in `${HOME}/.cache/HiCBricks/` unless otherwise configured.
