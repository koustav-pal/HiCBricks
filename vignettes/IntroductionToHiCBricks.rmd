...
having to deal with the structural complexity associated to dealing with
HDF files. Please note, HiCBricks is not meant to compete with other data
formats. Instead it is a library to better facilitate access to Hi-C data
stored in various formats. Therefore, it is not possible at this time to
create a direct mirror of for example, mcool files. mcool files, for the
uninitiated are a standard data format for the 4D nucleome project and better
facilitates the storage of very large experiments within a single data file.
HiCBricks, on the other hand stores the complete matrix, as opposed to the
upper triangle in mcools files. As you will see in this vignette, it is a
very simple task to import entire resolutions/normalizations from a
standard 4DN mcool file to a __Brick__ object.
...
outside (ROpenSci) Bioconductor. We welcome your feedback and suggestions as
to how we can improve the package. Please feel free to reach out to me via
email, github or the bioconductor support site.

### About Brick objects

HiCBricks creates HDF5 files. These files have the extension _.brick_ and will
be referred to as Brick objects from here on. The _Brick object_ implements a
HDF structure consisting of three layers. In no specific order, the first
houses Hi-C contact matrices as 2D matrices for every chromosome pair.
The second, houses the associated genomic ranges related to the Hi-C contact
matrix. Finally, the third layer contains any associated annotations that a
user may want to store within the Brick object. This information may be,
but is not limited to, TAD calls, Peak calls, RNA-seq expression data.
In principle, if the annotation information can be cast as a ranges object,
it can be stored in the Brick object.

### About file and path handling

To comply with Bioconductor standards, we make use of the `BiocFileCache`
package in the backend. We keep the Brick objects within the Cache directory.
...
Please note, that these are very large files, require a lot of time to
download, and as such this chunk of code was not executed during the
preparation of this vignette.
...
For the purposes of this vignette, we want to do TAD calls with the local
score differentiator (LSD) module and finally plot these TAD calls. We can
now go ahead and create a Brick corresponding for a single chromosome from
the downloaded mcool file and import data relevant to the cis-interaction
maps for that chromosome. 

```{r eval = FALSE}

Output.brick <- "H1-hESC-HiC-4DNFI7JNCNFB-10000-ICE-normalised-chr1.brick"
mcool <- mcoolName

CreateBrick_from_mcool(Brick = Output.brick, mcool = mcool,
    binsize = 10000, chrs = "chr1")
...
## Loading other datasets as Brick objects
...
```{r plot, fig.cap = "Figure 1. A normal heatmap without any transformations", fig.small = TRUE}
...
```{r plot2, fig.cap = "Figure 2. A normal heatmap with colours computed in log10 scale", fig.small = TRUE}
...
```{r plot3, fig.cap = "Figure 3. A normal heatmap with colours computed in log10 scale after capping values to the 99th percentile", fig.small = TRUE}
...
```{r plot4, fig.cap = "Figure 4. Same heatmap as before with colours computed in log10 scale after capping values to the 99th percentile with 45 degree rotation", fig.small = TRUE}
...
```{r plot5, fig.cap = "Figure 5. Same heatmap as previous, but now the heatmaps are wider than they are taller", fig.wide = TRUE}
...
```{r plot6, fig.cap = "Figure 6. Normal rectangular heatmap with colours computed in the log scale after capping values to the 99th percentile with TAD calls", fig.small = TRUE}
...
```{r plot7, fig.cap = "Figure 7. Normal rotated heatmap with colours computed in the log scale after capping values to the 99th percentile with TAD calls", fig.wide = TRUE}
...
```{r plot8, fig.cap = "Figure 8. A normal two sample heatmap with colours computed in log10 scale after capping values to the 99th percentile", fig.small = TRUE}
...
```{r plot9, fig.cap = "Figure 9. A normal two sample heatmap with colours computed in log10 scale on values until the 30th diagonal and capping these values to the 99th percentile.", fig.small = TRUE}
...
```{r plot10, fig.cap = "Figure 9. A rotated two sample heatmap with colours computed in log10 scale and capping these values to the 99th percentile.", fig.wide = TRUE}
...
```{r plot11, fig.cap = "Figure 10. A normal two sample heatmap with colours computed in log10 scale and capping these values to the 97th percentile, with TAD borders", fig.small = TRUE}
...
```{r plot12, fig.cap = "Figure 9. A rotated two sample heatmap with colours computed in log10 scale and capping these values to the 97th percentile, with non-truncated TAD borders", fig.wide = TRUE}
...
```{r plot13, fig.cap = "Figure 9. A rotated two sample heatmap with colours computed in log10 scale and capping these values to the 97th percentile, with truncated TAD borders", fig.wide = TRUE}
...
```{r plot14, fig.cap = "Figure 9. A rotated two sample heatmap with colours computed in log10 scale and capping these values to the 99th percentile.", fig.wide = TRUE}
