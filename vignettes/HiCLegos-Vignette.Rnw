...
HiCBlocks is a storage and retrieval library for high-resolution Hi-C data. 
...
files. Please note, HiCBlocks is not meant to compete with other data formats. 
...
storage of very large experiments within a single data file. HiCBlocks, on 
...
to a Block object.  
With HiCBlocks, in this vignette you will;
\begin{enumerate}
    \item Create a block for a single chromosomes
    \item do TAD calls on this block files
    \item keep the tad calls (or any ranges object) associated to the Block 
...
\section{Loading 4DN mcool files as Block objects}
...
Block\_list\_mcool\_normalisations() function. Please note, that this function 
does not list the normalisation weights available within the mcool file, 
but rather lists normalisation factors that HiCBlocks accepts from the mcool 
...
Block_list_mcool_normalisations(names.only = TRUE)
@
...
Block_list_mcool_normalisations(names.only = FALSE)
@
...
Block_list_mcool_resolutions(mcool = filename)
@


\noindent Once, you have viewed all of the listed resolutions, we can go 
ahead and create Block blocks from these files. Please note, that users cannot 
load multiple resolutions in a single Block file, this is because the aim of 
HiCBlocks is not to create an alternative storage format to already very 
...
Block object, this is not recommended because,
\begin{enumerate}
...
to separate the matrices chromosome by chromosome into different block 
...
calls. We can now go ahead and create a Block corresponding for a single 
chromosome from the downloaded mcool file and import data relevant to the 
cis-interaction maps for that chromosome.  

<<eval=FALSE, echo=TRUE >>=

Output.block <- "H1-hESC-HiC-4DNFI7JNCNFB-10000-ICE-
    normalised-chr1.block"
mcool <- mcoolName
...
Block_load_data_from_mcool(Block = Output.block, 
    mcool = mcool, 
    chr1 = "chr1", 
    chr2 = "chr1", 
    binsize = 10000,
...
\noindent The first function creates the basic Block structure, whereas the 
...
\section{ Loading other datasets as Block objects}

Currently, HiCBlocks only supports data import from 2D matrices 
...
Block.file <- Path_to_cached_file

Block_load_matrix(Block = Block.file, 
    chr1 = "chr19", 
    chr2 = "chr19",
...
Block_load_cis_matrix_till_distance(Block = Block.file, 
    chr = "chr19", 
    matrix.file = Matrix.file, 
    delim = " ", 
    distance = 100, 
...
\section{Accessing ranges objects in a block store}

HiCBlocks implementes different fetch/get methods. Users can list 
attributes of a Block object and they can fetch the same objects. 
Users have access to Ranges objects and matrix subset operations.

<<eval=TRUE, echo=TRUE >>=

Block.file <- system.file("extdata", 
    "test.hdf", 
    package = "HiCBlocks")

Block_list_rangekeys(Block.file)
@

\noindent This lists the available rangekeys in the Block file. Alternatively, 
tf you are interested in only the bintable associated to the Hi-C experiment, 
you can: 

<<eval=TRUE, echo=TRUE >>=

Block_get_bintable(Block.file)
@

\noindent Otherwise, you can retrieve the object using Block\_get\_ranges the 
method called by Block\_get\_bintable.

<<eval=TRUE, echo=TRUE >>=

Block_get_ranges(Block = Block.file, 
    rangekey = "Bintable")
@
...
testRun=Block_return_region_position(Block = Block.file, 
...
Block\_fetch\_range\_index which is called by Block\_return\_region\_position. 

<<eval=TRUE, echo=TRUE >>=

testRun=Block_fetch_range_index(Block = Block.file, 
    chr = "chr19", 
    start = 5000000, 
    end = 10000000)
@ 
...
\section{ Accessing matrices in a block store}
...
Values <- Block_get_values_by_distance(Block = Block.file, 
    chr = "chr19", 
    distance = 4)
@
...
Block_get_values_by_distance(Block = Block.file, 
    chr = "chr19", 
    distance = 4, 
...
HiCBlocks, implements word-alike retrieval of sub-matrices. That means, you 
can retrieve data using coordinate in coordinate like fashion.

<<eval=TRUE, echo=TRUE >>=

Sub.matrix <- Block_get_matrix_within_coords(Block = Block.file, 
...
Sub.matrix <- Block_get_matrix(Block = Block.file, 
    chr1 = "chr19", 
    chr2 = "chr19", 
...
Block.file <- system.file("extdata", 
    "test.hdf", 
    package = "HiCBlocks")
testRun <- Block_get_matrix_mcols(Block = Block.file, 
    chr1 = "chr19", 
    chr2 = "chr19", 
    what = "row.sums")
...
\subsection{ Check if a matrix has been loaded into the Block store}

<<eval=TRUE, echo=TRUE >>=

Block_matrix_isdone(Block = Block.file, 
    chr1 = "chr19", 
    chr2 = "chr19")
@
...
Block_matrix_issparse(Block = Block.file, 
    chr1 = "chr19", 
    chr2 = "chr19")
@
...
Block_matrix_maxdist(Block = Block.file, 
    chr1 = "chr19", 
    chr2 = "chr19")
@
...
Block_matrix_minmax(Block = Block.file, 
    chr1 = "chr19", 
    chr2 = "chr19")
@
...
Block_matrix_dimensions(Block = Block.file, 
    chr1 = "chr19", 
    chr2 = "chr19")
@
...
Block_matrix_filename(Block = Block.file, 
    chr1 = "chr19", 
    chr2 = "chr19")
@
...
HiCBlocks to show how different analysis procedures can take advantage 
of the HiCBlocks accessor functions.   
...
\noindent These TAD calls can also be stored along with the Block file.
...
Name <- paste("LSD",
    ...
    lookup_window,
    Chromosome,sep = "_")
Block_add_ranges(Block = Path_to_cached_file, 
    ranges = TAD_ranges, 
    rangekey = Name)
@
...
\section{ Fetching associated ranges from a Block file}

Since a block store is an on-disk database, it is possible to fetch ranges 
objects associated to the block store. Users can first list the available 
ranges objects. Once, they have identified the available rangekeys, 
users can fetch the required rangekeys. 

<<eval=TRUE, echo=TRUE >>=

Block_list_rangekeys(Block = Path_to_cached_file)
TAD_ranges <- Block_get_ranges(Block = Path_to_cached_file, 
    rangekey = Name)
@
...
\section{ Creating pretty heatmaps using HiCBlocks}

Using HiCBlocks functions, users can plot pretty Hi-C heatmaps. In 
...
Block_vizart_plot_heatmap(File = "chr19-5MB-10MB-normal.pdf", 
    Blocks = Block.file, 
    ...
    return.object=TRUE)
@
\caption{Simple heatmap generated with HiCBlocks.}
\label{fig:Fig1}
\end{figure}
...
Block_vizart_plot_heatmap(File = "chr19-5MB-10MB-normal2.pdf", 
    Blocks = Block.file, 
    x.coords = "chr19:5000000:10000000", 
    y.coords = "chr19:5000000:10000000", 
    FUN = Failsafe_log10, 
...
Block_vizart_plot_heatmap(File = "chr19-5MB-10MB-normal3.pdf", 
    Blocks = Block.file, 
    x.coords = "chr19:5000000:10000000", 
    y.coords = "chr19:5000000:10000000", 
    FUN = Failsafe_log10, 
...
Block_vizart_plot_heatmap(File = "chr19-5MB-10MB-normal4.pdf", 
    Blocks = Block.file, 
    x.coords = "chr19:5000000:10000000", 
    y.coords = "chr19:5000000:10000000", 
    FUN = Failsafe_log10, 
    ...
    return.object=TRUE)
@
\caption{Rotated heatmap created by HiCBlocks.}
\label{fig:Fig4}
\end{figure}
...
Block_vizart_plot_heatmap(File = "chr19-5MB-10MB-normal5.pdf", 
    Blocks = Block.file, 
    x.coords = "chr19:5000000:10000000", 
    y.coords = "chr19:5000000:10000000", 
    FUN = Failsafe_log10, 
...
Block_vizart_plot_heatmap(File = "chr19-5MB-10MB-normal6.pdf", 
    Blocks = Block.file, 
