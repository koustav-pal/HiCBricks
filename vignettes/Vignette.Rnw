...
You can download the file using curl. 
%<<eval=FALSE, echo=TRUE,tidy=TRUE >>=
%curl_download(url = "https://data.4dnucleome.org/
%files-processed/4DNFI7JNCNFB/@@download/4DNFI7JNCNFB.mcool",
%destfile = "H1-hESC-HiC-4DNFI7JNCNFB.mcool")
%@
It contains normalised Hi-C data on H1-hESC cells using the DpnII enzyme. 
...
Although it is possible to load all chromosomes in a single Lego object, 
...
\subsection{ Matrix utility functions}

There are several utility functions that a user may take advantage 
of to do various checks. 

\subsubsection{ Check if a matrix has been loaded into the Lego store}
...
\subsubsection{ Check if a matrix was defined as a sparse matrix}
...
\subsubsection{ Check the maximum distance until which a matrix was loaded}
...
\subsubsection{ Check if a matrix was defined}
...
\subsubsection{ Check the minimum and maximum values of a matrix}
...
\subsubsection{ Get the matrix dimensions, irrespective of the maxdist value}
...
\subsubsection{ Get the filename of a loaded matrix}
...
<<eval=FALSE, label=chr19-5MB-10MB-normal, include=FALSE, echo=FALSE>>=
...
\begin{figure}[htbp]
\centering
<<eval=TRUE,label=chr19-5MB-10MB-normal, fig=TRUE, echo=FALSE>>=
<<chr19-5MB-10MB-normal>>
@
...
<<eval=FALSE, label=chr19-5MB-10MB-normal2, include=FALSE, echo=FALSE>>=
...
<<eval=FALSE, label=chr19-5MB-10MB-normal3, include=FALSE, echo=FALSE>>=
...
\begin{figure}[htbp]
\centering
<<eval=TRUE,label=chr19-5MB-10MB-normal3, fig=TRUE, echo=FALSE>>=
<<chr19-5MB-10MB-normal3>>
@
...
<<eval=FALSE, label=chr19-5MB-10MB-normal4, include=FALSE, echo=FALSE>>=
...
\begin{figure}[htbp]
\centering
<<eval=TRUE,label=chr19-5MB-10MB-normal4, fig=TRUE, echo=FALSE>>=
<<chr19-5MB-10MB-normal4>>
@
...
\noindent But this looks ugly \ref{fig:Fig4}. To fix it we 
need to modify the 'width' and 
'height' as the rotated plots are broader than they are taller 
(Figure \ref{fig:Fig5}). 

<<eval=FALSE, label=chr19-5MB-10MB-normal5, include=FALSE, echo=FALSE>>=
...
\begin{figure}[htbp]
\centering
<<eval=TRUE,label=chr19-5MB-10MB-normal5, fig=TRUE, echo=FALSE>>=
<<chr19-5MB-10MB-normal5>>
@
\caption{Rotated Hi-C matrix with corrected plot-sizes.}
\label{fig:Fig5}
\end{figure}
...
<<eval=FALSE, label=chr19-5MB-10MB-normal6, include=FALSE, echo=FALSE>>=
...
\begin{figure}[htbp]
\centering
<<eval=TRUE,label=chr19-5MB-10MB-normal6, fig=TRUE, echo=FALSE>>=
<<chr19-5MB-10MB-normal6>>
@
...
<<eval=FALSE, label=chr19-5MB-10MB-normal7, include=FALSE, echo=FALSE>>=
Lego_vizart_plot_heatmap(File = "chr19-5MB-10MB-normal.pdf", 
    Legos = Lego.file, 
    tad.ranges = TAD_ranges,
    x.coords = "chr19:5000000:10000000", 
...
\begin{figure}[htbp]
\centering
<<eval=TRUE,label=chr19-5MB-10MB-normal7, fig=TRUE, echo=FALSE>>=
<<chr19-5MB-10MB-normal7>>
@
\caption{Rotated Hi-C matrix with TADs.}
\label{fig:Fig7}
\end{figure}
